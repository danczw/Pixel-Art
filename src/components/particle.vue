...
export default {
  ...
  methods: {
    effect() {
      ...
      let particleArray = [];
      let adjustX = window.innerWidth / 50; // move text on x-axis
      let adjustY = window.innerWidth / 50; // move text on y-axis
        ...
        for (let y = 0, y2 = textData.height; y < y2; y++) {
          for (let x = 0, x2 = textData.width; x < x2; x++) {
            if (textData.data[y * 4 * textData.width + x * 4 + 3] > 128) {
              ...
              particleArray.push(new Particle(positionX * 8, positionY * 8)); // change size of effect
            }
          }
        }
      }
      ...
      function connect() {
        ...
        for (let a = 0; a < particleArray.length; a++) {
          for (let b = a; b < particleArray.length; b++) {
            ...
            if (distance < 15) { // change distance limit to connect dots
              ...
              if (mouseDistance < mouse.radius / 2) {
                ...
                ctx.strokeStyle = `rgba(175, 255, 255, ${opacityValue})`;
              } else {
